                ...
                if ((p.Count() == 2) && !string.IsNullOrWhiteSpace(p[1]))
                {
                    try
                    {
                        switch (p[0].ToLower().Trim())
                        {
                            ...
                            case "#debit":
                                belob = p[1].Replace(",", ".");
                                Debit = double.Parse(belob.Trim(), System.Globalization.NumberStyles.AllowDecimalPoint);
                                break;

                            case "#kredit":
                                belob = p[1].Replace(",", ".");
                                Kredit = double.Parse(belob.Trim(), System.Globalization.NumberStyles.AllowDecimalPoint);
                                break;
