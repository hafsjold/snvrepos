...
namespace Pbs3060
{
        public static class UCInitializer
        {
            ...
            /// </summary>
            public static Uniconta.API.Service.Session CurrentSession;
        ...
        /// <returns></returns>
            ...
            public static Uniconta.API.Service.Session GetSession()
            {
            ...
            /// </summary>
            /// <returns></returns>
            public static Uniconta.API.Service.Session InitUniconta()
            {
                if (CurrentSession == null)
                {
                    var corasauConnection = new UnicontaConnection(APITarget.Live);
                    CurrentSession = new Uniconta.API.Service.Session(corasauConnection);
                }
                return CurrentSession;
            }
            ...
            public static void UnicontaLogin()
            {
                ...
                Console.WriteLine(string.Format("Medlem3060Service UnicontaLogin() failed with message: You do not have any access to company"));
            }
            ...
            static void UC_Login()
            {
                ...
                switch (errorCode)
                {
                    case ErrorCodes.Succes:
                        ...
                        Console.WriteLine(string.Format("Medlem3060Service simulatedloginButton() login as {0} OK", clsApp.UniContaUser));
                        break;

                    case ErrorCodes.UserOrPasswordIsWrong:
                    Console.WriteLine(string.Format("Medlem3060Service UC_Login() failed with message: Please Check Your Credentials & Try again !!!"));
                        break;

                    default:
                    Console.WriteLine(string.Format("Medlem3060Service UC_Login() failed with message: {0} {1}", "Unable to login", errorCode.ToString()));
                        break;
                }
            }
            ...
            static ErrorCodes SetLogin(string username, string password)
            {
                ...
                {
                Console.WriteLine(string.Format("Medlem3060Service SetLogin() failed with message: {0}", "Fatal Error"));
