...
class clsLobUserTilmelding {
	...
	private function __get($property) {
		switch ($property) {
			...
			case "okpage":
				return "https://www.puls3060.dk/lobmnu/pbstest/LobUserTilmelding.php?DoWhat=4&Card=eDank";
				break;

			case "errorpage":
				return "https://www.puls3060.dk/lobmnu/pbstest/LobUserTilmelding.php?DoWhat=4&Card=eDank&Err=1";
				break;
					...
					if ($this->_DBPuls3060->query($query, SQL_INIT, SQL_ASSOC)) {
						...
						if ($this->_DBPuls3060->prepare("INSERT INTO tblnrtildeling (lobid, nummer, lobtilmeldingid) VALUES(?, ?, ?)")) {
							...
							if ($this->_DBPuls3060->execute($data)) {
								...
								$body = $text_smarty->fetch("pbstest/LobUserTilmeldingMail.tpl");
