#!/usr/bin/perl -w
#----------------------------------------------------------------------------
#   daemon.pm - simple daemon object with logging functions
#
...
sub logto {
   my $self = shift;
   croak ("daemon::logto(): syntax: logto ([<filename>||//syslog||//debug]);") 
      unless (@_ <= 1);
...
sub new {
   ...
   $SIG{'QUIT'} = "daemon::inthandler_quit";
...
sub debug {
    ...
    $SIG{'INT'} = "daemon::inthandler_quit";
...
sub stop {
    my $self = shift;
    my $key;
    # print "QUITTING DAEMON - UNLINKING PID FILE\n";
...
C<daemon.pm> - simple daemon controlling object 

=head1 SYNOPSIS

C<my $daemon = new daemon;>
