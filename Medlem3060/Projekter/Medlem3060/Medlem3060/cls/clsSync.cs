...
namespace nsPuls3060
{
    ...
    public class clsSync
    {
        ...
        {
            var medlemlog = from l in Program.dbData3060.TblMedlemLog select l;
            Tblsync s;
            foreach (var l in medlemlog)
            {
                ...
                s = new Tblsync
                {
                    ...
                    Checksum = CRC32(l.Akt_dato)
                };
                action(s);
            }
            Program.dbData3060.SubmitChanges();
        }
        private void actionFakSync()
        {
        ...
        private void actionBetlin40Sync()
        {
        ...
        private int CRC32(int? Value)
        {
            if (Value == null) return 0;
            return CRC32((int)Value);
        }

        private int CRC32(int Value)
        {
            char[] c_arr = Value.ToString().ToCharArray();
            return CRC32(c_arr);
        }

        private int CRC32(DateTime? Value)
        {
            if (Value == null) return 0;
            return CRC32((DateTime)Value);
        }
        ...
        private int CRC32(char[] cValue)
        {
            byte[] Value = System.Text.Encoding.GetEncoding("windows-1252").GetBytes(cValue);
