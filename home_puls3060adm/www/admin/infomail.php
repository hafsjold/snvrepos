...
<?
//adm.puls3060.dk/mail_info/infomail.php?AktId=386&MaillistId=999
		...
		if ($this->_DBPuls3060->prepare($Query)) {
			if ($this->_DBPuls3060->execute($QueryDataArr)) {
				while ($this->_DBPuls3060->next(SQL_ASSOC)) {
					...
					if ($this->_aktid==386){
						$email_message->SetEncodedHeader("Subject","Hjælpere til Halloweenløbet");						
					}
					else{
						$email_message->SetEncodedHeader("Subject","Julehilsen fra Puls 3060");						
					}
