...
<?
	...
	$dbResult = pg_query($dbLink, "SELECT id, dato, navn FROM tbllob WHERE dato > now() ORDER BY dato asc LIMIT 1;");
    if ($row = pg_fetch_array($dbResult, NULL , PGSQL_ASSOC)) {
      $Lob_ref       = $row["id"];
      $Dato          = DatoFormat($row["dato"]);
      $MailListeNavn = $row["navn"];
	}
	
	if ($MaillistId == 998) {
	  $Query="SELECT opretmailliste('" . $MailListeNavn . "', 1, 0) AS MaillistId;";
	...
	while($row = pg_fetch_array($dbResult, NULL , PGSQL_ASSOC))
	{
        ...
        if ($Lob_ref != 83) {
    		$subject="Elektronisk tilmelding til Espergærdeløbet";
        	$text = get_file('tilmelding.txt');
        	$html = get_file('tilmelding.htm');
		}
		else {
    		$subject="Elektronisk tilmelding til Halloweenløb";
        	$text = get_file('tilmelding_83.txt');
        	$html = get_file('tilmelding_83.htm');
		}
