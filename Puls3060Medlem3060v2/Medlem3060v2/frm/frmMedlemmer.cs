    ...
    public partial class FrmMedlemmer : Form
    {
        ...
        private void I_Postnr_Leave(object sender, EventArgs e)
        {
            if (I_Postnr.Text.Length == 4)
            {
                try
                {
                    var medlem = (from i in Program.dbData3060.tblMedlem where i.Postnr == I_Postnr.Text select i).First();
        ...
        private void U_Postnr_Leave(object sender, EventArgs e)
        {
            if (U_Postnr.Text.Length == 4)
            {
                try
                {
                    var medlem = (from i in Program.dbData3060.tblMedlem where i.Postnr == U_Postnr.Text select i).First();
        ...
        private int tblMedlemsNextval()
        {
            try
            {
                int maxNr2 = (from k in Program.dbData3060.tblMedlem orderby k.Nr descending select k.Nr).First();
                int maxNr = ++maxNr2;
                return maxNr;
            }
            catch (Exception)
