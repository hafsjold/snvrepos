...
namespace nsPbs3060v2
{
    public class clsKontingent
    {
        ...
        public clsKontingent(dbData3060 p_dbData3060, DateTime startdato, int? Nr)
        {
            DateTime? fodtdato;
            try
            {
                fodtdato = (from m in p_dbData3060.tblMedlem where m.Nr == Nr select m.FodtDato).First();
        ...
        public clsKontingent(dbData3060 p_dbData3060, DateTime startdato, DateTime? fodtdato)
        {
            this.BeregnKontingent(p_dbData3060, startdato, fodtdato);
        }

        private void BeregnKontingent(dbData3060 p_dbData3060, DateTime startdato, DateTime? fodtdato) 
        {
            int alder;
            if (fodtdato == null) 
            {
