...
namespace BackupHDC
{
    public partial class frmBackup : Form
    {
        ...
        private string regKey = @"Software\Hafsjold\HDA\Start";
        ...
        public frmBackup()
        {
            ...
            if (masterKey == null)
            {
                ...
                masterKey = masterKeyCreate.CreateSubKey(@"Hafsjold\HDA\Start");
            }
        ...
        private string execZip()
        {
            ...
            string Target = BackupDir.TrimEnd('\\') + @"\SUMMAHDC" + fdt + @".zip";
            ...
            try
            {
                ...
                if (codeExit == 0)
                {
                    WriteToEventLog("BackupHDC ended normaly", EventLogEntryType.Information, 49101);
                }
                else
                {
                    WriteToEventLog("BackupHDC Error-1", EventLogEntryType.Error, 49102);
                }
            }
            catch
            {
                WriteToEventLog("BackupHDC Error-2", EventLogEntryType.Error, 49103);
            }
            return Target;
        ...
        {
            FileStream ts = new FileStream(@"BackupHDALog.txt", FileMode.OpenOrCreate, FileAccess.Write, FileShare.None);
