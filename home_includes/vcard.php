	...
	for ($j=0;$j<count($lines);$j++) {
		...
		for($i = 0; $i < $linlen; $i++) {
			...
			if ( ($dec == 32) && ($i == ($linlen - 1)) ) { // convert space at eol only
				$c = "=20";
			} elseif ( ($dec == 61) || ($dec < 32 ) || ($dec > 1126) ) { // always encode "\t", which is *not* required
...
class vCard {
	...
	function setName($family="", $first="", $additional="", $prefix="", $suffix="") {
		...
		if ($this->properties["FN"]=="") $this->setFormattedName(removedoublespace(trim("$prefix $first $additional $family $suffix")));
	}

	function setBirthday($date) { // $date format is YYYYMMDD
	...
	function setCategories($categories) {
		$this->properties["CATEGORIES"] = trim($categories);
	}
	...
	function setOrg($company="", $department="") {
		$this->properties["ORG"] = trim("$company;$department");
	}

	function setTitle($title) {
		$this->properties["TITLE"] = trim($title);
	}
	...
	function getVCard() {
		...
		$text.= "REV:".date("m-d-Y")."T".date("H:i:s")."\r\n";
		$text.= "END:VCARD\r\n";
		return $text;
	}
