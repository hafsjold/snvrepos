...
class php_http_request {
	...
	var $protocol = 'HTTP'; //or HTTPS (requires curl extension)
...
class php_http_client_generic extends php_http_request {
	...
	var $response = null;
	/** @var boolean True if cURL is to be used  */
	var $doUseCURL = false;
	...
	*/
	...
	/**
	* Specifies whether cURL should be used to retrieve data
	* @param boolean True if cURL is to be used
	*/
	function useCURL($truthVal) {
		$this->doUseCURL = $truthVal;
	} //useCURL
	...
	*/
	function &send($message) {
		...
		if  (!$this->doUseCURL) {
					...
					switch ($readState) {
						...
						case HTTP_READ_STATE_HEADERS:
							if (trim($data) == '') { //end of headers is signalled by a blank line
								//check location header for redirection
								$readState = HTTP_READ_STATE_BODY;
							}
							else {
								//check for redirections
								if (substr(strtolower($data),0,9) == "location:") { 
									return  $this->redirect(trim(substr($data, 10)), $message);
								}
								else if (substr(strtolower($data),0,17) == "content-location:") { 
									return $this->redirect(trim(substr($data, 18)), $message);
								}
			...
			return $this->invokeCURL($message);
		}
	} //send
	...
	*/
	function &invokeCURL ($message) {
		if (function_exists('curl_init')) {
			...
			curl_setopt($curlObj, CURLOPT_HTTPHEADER, $curlHeaderArray);
			
			curl_setopt($curlObj, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curlObj, CURLOPT_FOLLOWLOCATION, 1);
			curl_setopt($curlObj, CURLOPT_URL, $this->requestPath);
			curl_setopt($curlObj, CURLOPT_PORT, $this->connection->port);
			curl_setopt($curlObj, CURLOPT_CONNECTTIMEOUT, $this->connection->timeout);
			
			if ($this->protocolVersion == '1.0') {
				curl_setopt($curlObj, CURLOPT_HTTP_VERSION, CURLOPT_HTTP_VERSION_1_0);
			}
			else {	
				curl_setopt($curlObj, CURLOPT_HTTP_VERSION, CURLOPT_HTTP_VERSION_1_1);
			}
			...
			$messageBody = curl_exec($curlObj);
			
			// close curl resource, and free up system resources
			curl_close($curlObj);
			
			$this->response->message = $messageBody;
	
			return $this->response;
		}
		else {
			//cURL not installed; https protocol not supported
			HTTPExceptions::raiseException(HTTPS_UNSUPPORTED_ERR, ('HTTPS Unsupported Error - You must install the cURL extension.'));
		}
	} //invokeCURL
	
	/**
	* Handles Location or Content-Location based header redirection
	* @param string The url of the redirection
	* @param string The message of the post
	* @return string The decoded data
	*/
	function &redirect($url, $message) {
		if (strpos(strtolower($url), 'https://') !== false) {
			//use cURL with original URL,
			//to avoid funky redirection problems
			$this->useCURL(true);
			...
			$this->connection = new php_http_connection(($this->protocol . '://' . $host), $path, $this->port, $this->timeout);
...
class php_http_connection {
	...
	*/
	function formatHost($hostString) {	
		if (substr(strtolower($hostString), 0, 7) == 'http://') {
			$hostString = substr($hostString, 7);
		}
		else if (substr(strtolower($hostString), 0, 8) == 'https://') {
			$hostString = substr($hostString, 8);
		}
