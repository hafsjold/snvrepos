...
/**
 * @package QuickPay
 * @abstract This API is used to interact with QuickPay payment gateway. Use this API if implementing QuickPay in a PHP enviroment. Requires PHP to be compiled with CURL and XML support.
...
echo 'Status request: ' . $qpstatText["" . $eval['qpstat'] . ""] . '<br />';
echo "<pre>";
var_dump($eval);
echo "</pre>";
} else {
...
class quickpay {
    ...
    */
	public function quickpay() {
		...
		if(!extension_loaded('curl')) {
			throw new Exception("The \"curl\" extension must be installed on your server<br />");
		}
		if(!extension_loaded('xml')) {
			throw new Exception("The \"xml\" extension must be installed on your server<br />");
		}
	}
    ...
    * @return mixed    Return an array with statuscode if communcation with quickpay succeeded - otherwise false
	...
	public function status() {
		if ((isset($this->ordernum)) && ($this->ordernum > 0)) {
			$this->checkVars('stat2');
		}
		else {
			$this->checkVars('stat');
		}
		...
		if ((isset($this->ordernum)) && ($this->ordernum > 0)) {
			$postData[ 'ordernum' ] = $this->ordernum;
			$postData[ 'md5check' ] = md5($this->msgtype.$this->merchant.$this->ordernum.$this->md5checkword);
		}
		else {
			$postData[ 'transaction' ] = $this->transaction;
			$postData[ 'md5check' ] = md5($this->msgtype.$this->merchant.$this->transaction.$this->md5checkword);
    ...
    */
	private function checkVars($type) {
		...
		$stat2Vars = 'md5checkword,msgtype,merchant,ordernum';
		...
		if (is_array($vars)) {
			for ($i = 0; $i < count($vars); $i++) {
				if (!isset($this->$vars[$i])) {
					throw new Exception("You need to set " . $vars[$i] . " - example: \$obj->set_" . $vars[$i] . "(&lt;value&gt;);");
				}
			}
		}
	}
    ...
    */
	private function startElement($parser, $name, $attrs) {
		...
		elseif ($name == "history") {
			$this->arr['history'][$attrs['time']] = $attrs;
		}
	}
	/**
    * XML helper function
    * @return void
    */
	private function endElement($parser, $name)    {/* Vi bruger ikke noget fra endElementet.*/}
	/**
    * XML helper function
    * @return void
    */
	private function characterData($parser, $data) {/* Vi har ingen CharacterData.*/}
